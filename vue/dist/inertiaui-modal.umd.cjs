(function(global, factory) {
	...
	var _sfc_main$4 = {
		...
		setup(__props) {
			...
			const removeInterceptor = _inertiajs_vue3.http.onRequest(requestInterceptor);
			(0, vue.onUnmounted)(() => removeInterceptor());
	...
	function prefetch(href, options = {}) {
		...
		const request = _inertiajs_vue3.http.getClient().request({
			url,
			method,
			data: mergedData,
	...
	var Modal = class {
		constructor(component, response, config, onClose, afterLeave) {
			...
			this.reload = (options = {}) => {
				...
				const data = options.data ?? {};
				options.onStart?.();
				_inertiajs_vue3.http.getClient().request({
	...
	function pushFromResponseData(responseData, config = {}, onClose = null, onAfterLeave = null) {
		...
		return _inertiajs_vue3.router.resolveComponent(responseData.component).then((component) => push((0, vue.markRaw)(component), responseData, config, onClose, onAfterLeave));
	}
	...
	var withInertiaModal = (app) => {
		...
		if (originalRender) app._component.render = () => (0, vue.h)(_sfc_main$4, () => originalRender());
	};
	...
	*/
	...
	exports.withInertiaModal = withInertiaModal;
});
