...
<template>
    <Modal ref="modalRef" v-slot="{ close, getParentModal }" @greeting="greetingRef = $event">
        <div class="">
            <h2 class="text-lg font-medium text-gray-900">Create Role</h2>
            <p v-if="greetingRef" data-testid="greeting" class="text-sm text-gray-500" v-text="greetingRef" />
            <p v-if="headerValue" data-testid="headerValue" class="text-sm text-gray-500" v-text="headerValue" />
            <p v-if="name" data-testid="name" class="text-sm text-gray-500" v-text="name" />
        </div>

        <button type="button" @click.prevent="getParentModal().emit('message', 'Hello from child')">Push message to parent</button>
        ...
        <form class="mt-8 space-y-6" @submit.prevent="submit">
            ...
            <div class="flex items-center justify-end">
                <ModalLink max-width="sm" href="#another-local-modal" class="mr-auto text-sm text-pink-500"> What's that? </ModalLink>
