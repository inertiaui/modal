...
# ── args ────────────────────────────────────────────────────────────────
[ $# -eq 1 ] || die "Usage: $0 <version>  (e.g. 3.0.2 or 3.1.0-beta1)"
...
MAJOR="${VERSION%%.*}"
[ "$MAJOR" = "3" ] || die "This script only handles 3.x releases (got $VERSION). Run it from the matching branch."
...
[ "$BRANCH" = "3.x" ] || die "3.x releases must be cut from '3.x' (currently on '$BRANCH')"
...
# ── confirm ─────────────────────────────────────────────────────────────
# 3.x is the current major — stable releases mark "latest" on GitHub and publish
# under the default npm dist-tag. Pre-releases never mark as latest.
MARK_LATEST=true
...
info "update-changelog workflow will commit CHANGELOG.md updates back to the 3.x branch."
